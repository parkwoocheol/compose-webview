...
actual fun WebView.platformLoadUrl(url: String, additionalHttpHeaders: Map<String, String>) = this.loadUrl(url, additionalHttpHeaders)
...
actual fun WebView.platformPostUrl(url: String, postData: ByteArray) = this.postUrl(url, postData)
actual fun WebView.platformEvaluateJavascript(script: String, callback: ((String) -> Unit)?) = this.evaluateJavascript(script, callback)
...
actual fun WebView.platformScrollTo(x: Int, y: Int) = this.scrollTo(x, y)
actual fun WebView.platformScrollBy(x: Int, y: Int) = this.scrollBy(x, y)
actual fun WebView.platformSaveWebArchive(filename: String) = this.saveWebArchive(filename)

actual fun WebView.platformAddJavascriptInterface(obj: Any, name: String) = this.addJavascriptInterface(obj, name)
